  ...
  describe 'POST #create' do
    context 'with valid params' do
      before do
        ...
        allow(task).to receive(:status=).with('pending')
        allow(task).to receive(:save).and_return(true)

        allow(NotificationClient).to receive(:notify)
      end

      it 'creates a new Task, notifies the user and schedules the scraping job' do
        ...
        expect(ScrapingJob).to have_been_enqueued.with(task.id)
      end
    end
    ...
    context 'with invalid params' do
      before do
        ...
        allow(task).to receive(:user_id=).with(user.id)
        allow(task).to receive(:status=).with('pending')
        allow(task).to receive(:save).and_return(false) # Simula falha
      end
      ...
      it "returns an unprocessable_content response" do
        ...
        expect(ScrapingJob).not_to have_been_enqueued
        expect(NotificationClient).not_to receive(:notify)
      end
    end
  end
